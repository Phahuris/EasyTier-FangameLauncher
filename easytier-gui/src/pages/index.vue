...
const fangameAllowed = ref(false)
const fangameDisplayName = ref('')
...
const canStartParty = computed(() => {
  return !!(fangamePath.value || '').trim() && fangameAllowed.value === true
})
...
function showCommandHelp() {
  if (uiLang.value === 'fr') addLog('Commandes: /battle {pseudo}  |  /trade {pseudo}', 'info')
  else addLog('Commands: /battle {nickname}  |  /trade {nickname}', 'info')
}
...
async function sendChat() {
  ...
  if (battleMatch || tradeMatch) {
    ...
    try {
      ...
      if (!names.includes(target.toLowerCase()) && names.length > 0) {
        // autorise quand meme l'envoi jeu local ; avertit si pseudo inconnu cote launcher
        addLog(uiLang.value === 'fr' ? 'Utilisateur invalide' : 'Invalid user', 'warn')
        return
      }
      if (peers.length > 0) {
      ...
      try {
        await invoke('write_game_command', { gamePath: (fangamePath.value || '').trim(), command: '/' + plugin + ' ' + target })
      } catch (e) {
...
async function refreshPeers() {
  ...
  try {
    ...
    if (changed) { /* peers mis a jour silencieusement */ } catch {
...
async function browseFangame() {
  try {
    ...
    fangameAllowed.value = false
    fangameDisplayName.value = info.game_title || ''

    if (!info.ok || !(fangamePath.value || '').trim()) {
      fangameAllowed.value = false
      addLog((info.game_title || 'Fangame') + (uiLang.value === 'fr' ? ' non pris en charge' : ' not supported'), 'warn')
      return
    }

    const lang = (uiLang.value || 'fr').toString()
    try {
      const res = await invoke<any>('is_fangame_allowed_lang', { path: fangamePath.value, lang })
      if (res?.allowed) {
        fangameAllowed.value = true
        fangameDisplayName.value = res.display_name || info.game_title || 'Pokemon Infinite Fusion'
        addLog((uiLang.value === 'fr' ? 'Fangame valide : ' : 'Fangame valid: ') + fangameDisplayName.value, 'ok')
      } else {
        fangameAllowed.value = false
        const shown = info.game_title || res?.title || 'Fangame'
        fangameDisplayName.value = shown
        addLog(shown + (uiLang.value === 'fr' ? ' non pris en charge' : ' not supported'), 'warn')
      }
    } catch (e) {
      fangameAllowed.value = false
      addLog((info.game_title || 'Fangame') + (uiLang.value === 'fr' ? ' non pris en charge' : ' not supported'), 'warn')
...
async function ensureFangameReady(): Promise<boolean> {
  ...
  if (!path) {
    addLog(uiLang.value === 'fr' ? 'Choisis un fangame' : 'Select a fangame', 'warn')
    return false
  }
  if (!fangameAllowed.value) {
    const shown = fangameTitle.value || fangameDisplayName.value || 'Fangame'
    addLog(shown + (uiLang.value === 'fr' ? ' non pris en charge' : ' not supported'), 'warn')
    return false
  }
  const lang = (uiLang.value || 'fr').toString()
  try {
    let res = await invoke<any>('is_fangame_allowed_lang', { path, lang })
    if (!res?.allowed) {
      const shown = res?.title || fangameTitle.value || 'Fangame'
      fangameAllowed.value = false
      addLog(shown + (uiLang.value === 'fr' ? ' non pris en charge' : ' not supported'), 'warn')
      return false
    }
    fangameAllowed.value = true
    if (!res?.plugins_ok) {
      const msg = await invoke<string>('install_fgl_plugins', { gamePath: path, lang })
      addLog(String(msg), 'ok')
      res = await invoke<any>('verify_fgl_plugins', { gamePath: path, lang })
      if (!res?.plugins_ok) {
        addLog(uiLang.value === 'fr' ? 'Plugins manquants' : 'Missing plugins', 'warn')
        return false
      }
    }
    return true
...
async function stopHost() {
  ...
  try {
    ...
    try {
      const root = (fangamePath.value || '').trim()
      if (root) await invoke('cleanup_fgl_temp', { gamePath: root })
    } catch { /* ignore */ }
    ...
    <div v-show="activeTab === 'create'" class="fgl-panel">
      ...
      <div v-if="!isNetworkActive" class="fgl-card">
        ...
        <div class="fgl-grid fgl-grid-create">
          ...
          <div class="fgl-field fgl-field-half">
            ...
            <div v-if="fangameTitle" class="fgl-fangame-title"><span :class="fangameAllowed ? 'fangame-name-ok' : 'fangame-name-bad'">{{ fangameDisplayName || fangameTitle || '' }}</span></div>
        ...
        <div class="fgl-actions">
          ...
          <button type="button" class="fgl-btn green" :disabled="isBusy || !canStartParty" @click="startHost" :disabled="isBusy || !canStartParty">{{ s.startHost }}</button>
      ...
      <div v-else class="fgl-card fgl-card-running">
        ...
        <div class="fgl-actions">
          ...
          <button type="button" class="fgl-btn red" :disabled="isBusy || !canStartParty" @click="stopHost">{{ s.stopHost }}</button>
    ...
    <div v-show="activeTab === 'join'" class="fgl-panel">
      ...
      <div v-if="!isNetworkActive" class="fgl-card">
        ...
        <div class="fgl-actions">
          ...
          <button type="button" class="fgl-btn blue" :disabled="isBusy || !canStartParty" @click="startJoin" :disabled="isBusy || !canStartParty">{{ s.doJoin }}</button>
