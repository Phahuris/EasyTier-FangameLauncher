...
async function ensureFglReady(): Promise<boolean> {
  ...
  try {
    let res = await invoke<any>('is_fangame_allowed_lang', { path: root, lang })
    if (!res?.allowed) {
      addLog((uiLang.value === 'fr' ? 'Fangame refuse: ' : 'Fangame denied: ') + (res?.reason || ''), 'warn')
      return false
    }
    if (!res?.plugins_ok) {
      const msg = await invoke<string>('install_fgl_plugins', { gamePath: root, lang })
      addLog(String(msg), 'ok')
      res = await invoke<any>('verify_fgl_plugins', { gamePath: root, lang })
      if (!res?.plugins_ok) {
        addLog('Plugins manquants: ' + JSON.stringify(res?.plugins_missing || []), 'warn')
        return false
      }
    } else {
      addLog(uiLang.value === 'fr' ? 'Plugins deja presents' : 'Plugins already present', 'ok')
    }
    addLog((uiLang.value === 'fr' ? 'Fangame OK: ' : 'Fangame OK: ') + (res?.display_name || 'infinitefusion'), 'ok')
    return true
  } catch (e) {
    addLog('[FGL] ' + String(e), 'warn')
    return false
  }
}
async function checkFangameAllowed(): Promise<boolean> {
  const root = (fangamePath.value || '').trim()
  if (!root) {
    addLog(uiLang.value === 'fr' ? 'Aucun fangame selectionne' : 'No fangame selected', 'warn')
    return false
  ...
  try {
    const res = await invoke<any>('is_fangame_allowed', { path: root })
    if (!res?.allowed) {
      addLog((uiLang.value === 'fr' ? 'Fangame refuse: ' : 'Fangame denied: ') + (res?.reason || ''), 'warn')
      return false
    }
    addLog((uiLang.value === 'fr' ? 'Fangame OK: ' : 'Fangame OK: ') + (res?.display_name || res?.title || ''), 'ok')
    return true
  } catch (e) {
...
async function sendChat() {
  ...
  if (battleMatch || tradeMatch) {
    ...
    addLog(name + ' : /' + plugin + ' ' + target, 'chat')
    ...
    try {
      ...
      if (peers.length === 0) {
        addLog(uiLang.value === 'fr' ? 'Aucun peer' : 'No peers', 'warn')
        return
      }
      await invoke('chat_send_cmd', { pseudo: name, plugin, action: target, peers })
      await writeGameCommand('/' + plugin + ' ' + target)
      addLog((uiLang.value === 'fr' ? 'Commande envoyee : ' : 'Command sent: ') + plugin + ' -> ' + target, 'ok')
    } catch (e) {
      addLog('[Chat] CMD: ' + String(e), 'warn')
    }
    return
  }
...
async function ensureFangameReady(): Promise<boolean> {
  ...
  try {
    ...
    addLog((uiLang.value === 'fr' ? 'Fangame OK: ' : 'Fangame OK: ') + (res?.display_name || res?.title || 'infinitefusion'), 'ok')
    return true
  } catch (e) {
    addLog('Fangame: ' + String(e), 'warn')
    return false
