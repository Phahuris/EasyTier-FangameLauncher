...
const canStartParty = computed(() => !!(fangamePath.value || '').trim() && fangameAllowed.value === true)
  ...
  try {
    ...
    try {
      ...
      if (res?.allowed) {
        ...
        fangameAllowed.value = false
        const shown = info.game_title || String(res?.title || 'Fangame')
...
async function ensureFangameReady(): Promise<boolean> {
  ...
  try {
    ...
    if (!res?.allowed) {
      fangameAllowed.value = false
      const shown = String(res?.title || fangameTitle.value || 'Fangame')
      addLog(shown + (uiLang.value === 'fr' ? ' non pris en charge' : ' not supported'), 'warn')
      return false
    }
    fangameAllowed.value = true
    if (!res?.plugins_ok) {
      await invoke<string>('install_fgl_plugins', { gamePath: path, lang })
...
async function startJoin() {
  ...
  try {
    ...
    if (!clientRunning.value) {
      ...
      joinStatus.value = 'DEMO ├öC├Â join simule (pas de backend)'
...
onMounted(async () => {
  ...
  try {
    ...
    const unlistenChat = await listen<any>('chat_message', (event) => {
      ...
      console.log('[CHAT RX GUI] Message reÔö£┬║u:', pkt)
      ...
      if (pkt.type === 'chat' || pkt.kind === 'chat') {
        ...
        addLog(
          ...
          `[CMD] ${pkt.pseudo || 'Anonyme'} ├ö├Ñ├å ${pkt.plugin || ''} / ${pkt.action || ''}`
    ...
    console.warn('[CHAT] Ôö£ecoute chat indisponible:', e)
  ...
  try {
    ...
    addLog('DÔö£┬«marrage sans backend Tauri ├öC├Â UI seule')
