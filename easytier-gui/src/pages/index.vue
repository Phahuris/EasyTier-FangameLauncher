...
async function installFglForCurrentGame() {
  const root = (fangamePath.value || '').trim()
  ...
  try {
    ...
    addLog('[Install FGL] ' + String(e), 'warn')
    return false
  }
}
...
async function cleanupFglTemp() {
  const root = (fangamePath.value || '').trim()
  if (!root) return
  try {
    const n = await invoke<number>('cleanup_fgl_temp', { gamePath: root })
    if (n > 0) addLog('FGL temp cleaned: ' + n, 'ok')
  } catch { /* ignore */ }
}
