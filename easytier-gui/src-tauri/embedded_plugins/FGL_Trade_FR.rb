...
module FGLTrade
  ...
  def self.my_name
    (FGL.player_display_name rescue nil) || ($Trainer.name.to_s rescue "Joueur")
  end
  ...
  def self.show_prep_phase(text = nil)
    ...
    text = _INTL("Preparation de l'echange...") if text.nil?
    msg = nil
    begin
  ...
  def self.can_send_trade?(target_id)
    ...
    if (now - @last_sent) < COOLDOWN
      return [false, _INTL("Attends un peu avant de renvoyer une demande d'echange.")]
    end
    if busy?
      return [false, _INTL("Tu es deja en echange.")]
    end
    begin
      if defined?(FGLBattle) && (FGLBattle.busy? || FGLBattle.active?)
        return [false, _INTL("Tu es deja en combat.")]
      end
    rescue; end
    if peer_is_busy?(target_id)
      return [false, _INTL("Cette personne est occupee ou a deja une demande en cours.")]
    end
    [true, nil]
  end
  ...
  def self.choose_party_index
    ...
    begin
      pbFadeOutIn(99999) {
        ...
        screen.pbStartScene(_INTL("Choisir un Pokemon a echanger."), false)
        loop do
          ...
          next if pkmn.nil?
          cmds = [_INTL("Echanger"), _INTL("Resume"), _INTL("Retour")]
          cmd = -1
          begin
            cmd = scene.pbShowCommands(_INTL("Que faire avec {1} ?", pkmn.name), cmds)
          rescue
            begin
              cmd = Kernel.pbMessage(_INTL("Que faire avec {1} ?", pkmn.name), cmds, 2)
            rescue
              cmd = 0
            end
          end
  ...
  def self.trade_player(target_name)
    ...
    if peer.nil?
      force_msg_bottom_system!
      pbMessage("\\wd" + _INTL("Joueur introuvable : {1}", target_name)) rescue nil
      return
    end
  ...
  def self.send_request(target_id, target_name)
    ...
    force_msg_bottom_system!
    pbMessage("\\wd" + _INTL("Demande d'echange envoyee a {1}.", target_name)) rescue nil
    true
  end
  ...
  def self.accept_request(tid, from_id, from_name)
    ...
    ok = false
    begin
      ok = pbConfirmMessage("\\wd" + _INTL("{1} veut echanger un Pokemon. Accepter ?", from_name))
    rescue
      ok = true
    end
    if ok
  ...
  def self.start_trade_flow(_initiator)
    show_prep_phase(_INTL("Preparation de l'echange..."))
    idx = choose_party_index
    if idx.nil?
      force_msg_bottom_system!
      pbMessage("\\wd" + _INTL("Echange annule.")) rescue nil
    ...
    show_prep_phase(_INTL("Chargement de l'echange..."))
    remote_pkmn = nil
    150.times do
      Graphics.update rescue nil
      Input.update rescue nil
    ...
    if remote_pkmn.nil?
      force_msg_bottom_system!
      pbMessage("\\wd" + _INTL("L'autre joueur n'a pas repondu a temps.")) rescue nil
      cleanup
      return
    end
    show_prep_phase(_INTL("Echange en cours..."))
    ...
    unless confirmed
      force_msg_bottom_system!
      pbMessage("\\wd" + _INTL("Echange non confirme.")) rescue nil
      cleanup
      return
    end
    given = nil
    ...
    begin
      if defined?(PokemonTrade_Scene) || defined?(PokemonTradeScene)
        ...
        pbMessage("\\wd" + _INTL("Tu as echange {1} contre {2} de {3} !",
          (given.name rescue "Pokemon"),
          (remote_pkmn.name rescue "Pokemon"),
          @remote_name.to_s)) rescue nil
      end
    rescue
      pbMessage("\\wd" + _INTL("Echange termine !")) rescue nil
    end
    force_save_play_slot
    cleanup
  end
    ...
    if @busy && !@active && @trade_id
      ...
      if ans
        if ans.include?("ACCEPT")
          ...
          force_msg_bottom_system!
          pbMessage("\\wd" + _INTL("Demande d'echange refusee.")) rescue nil
          cleanup
        end
      end
      age = 0
      ...
      if age > 30
        force_msg_bottom_system!
        pbMessage("\\wd" + _INTL("Demande d'echange expiree.")) rescue nil
        cleanup
      end
      return
    end
