...
module FGLBattle
  ...
  def self.can_send_challenge?(target_id)
    ...
    if (now - @last_chal_sent) < CHAL_COOLDOWN
      return [false, _INTL("Attends un peu avant de renvoyer un defi.")]
    end
    if busy? || active?
      return [false, _INTL("Tu es deja en combat ou occupe.")]
    end
  ...
  def self.challenge_player(target_name)
    ...
    if peer.nil?
      force_msg_bottom!
      begin; pbMessage("\\wd" + _INTL("Joueur introuvable : {1}", target_name)); rescue; end
      return
    end
    tid, tname, _o = peer
    mid = my_id
    ...
    force_msg_bottom!
    pbMessage("\\wd" + _INTL("Demande de combat envoyee a {1}.", tname)) rescue nil
  end
  ...
  def self.poll_incoming
    ...
    force_msg_bottom!
    ok = false
    begin; ok = pbConfirmMessage("\\wd" + _INTL("{1} vous defie en combat ! Accepter ?", from_name)); rescue; ok = false; end
    if ok
  ...
  def self.poll_response
    ...
    if parts[0] == "DECLINE"
      force_msg_bottom!
      begin; pbMessage("\\wd" + _INTL("Defi refuse par {1}.", @pending[:name].to_s)); rescue; end
      @pending = nil
      return
    end
    if parts[0] == "ACCEPT"
  ...
  def self.wait_and_start(bid, other_id, other_name, seed)
    ...
    @shared_seed = seed
    force_msg_bottom!
    begin; pbMessage("\\wd" + _INTL("Preparation du combat...")); rescue; end
    heal_party_silent
    snapshot_items
    ...
    if foe.nil? || foe.length == 0
      ...
      force_msg_bottom!
      begin; pbMessage("\\wd" + _INTL("Impossible de demarrer le combat.")); rescue; end
      return
    end
    @battle_id = bid
    @remote_id = other_id
  ...
  def self.wait_remote_human(battle)
    ...
    20000.times do
      ...
      if !shown
        shown = true
        show_wait_msg(battle, _INTL("Attente d'une action de {1}...", rname))
      else
        hide_command_ui(battle)
        fix_message_bottom(battle)
      end
  ...
  def self.wait_remote_force_switch(battle)
    ...
    30000.times do
      ...
      if !shown
        shown = true
        show_wait_msg(battle, _INTL("{1} choisit un Pokemon...", rname))
      else
        hide_command_ui(battle)
        fix_message_bottom(battle)
      end
  ...
  def self.start_human_vs_human(foe_party, foe_name, seed)
    foe_name = "Joueur" if foe_name.to_s == ""
    ...
    begin
      ...
      cleanup_after_battle
      force_msg_bottom!
      begin; pbMessage("\\wd" + _INTL("Erreur de combat.")); rescue; end
    ensure
      heal_party_silent
      restore_items
      cleanup_after_battle
  ...
  def self.show_end_messages(decision, foe_name)
    ...
    begin
      if @forfeit_local
        pbMessage("\\wd" + _INTL("Vous avez declare forfait.")) rescue nil
        pbMessage("\\wd" + _INTL("Vous avez perdu contre {1}.", name)) rescue nil
      elsif @forfeit_remote
        pbMessage("\\wd" + _INTL("{1} a declare forfait.", name)) rescue nil
        pbMessage("\\wd" + _INTL("Vous avez gagne contre {1}.", name)) rescue nil
      elsif decision == 1
        pbMessage("\\wd" + _INTL("Vous avez gagne contre {1}.", name)) rescue nil
      elsif decision == 2
        pbMessage("\\wd" + _INTL("Vous avez perdu contre {1}.", name)) rescue nil
      else
        pbMessage("\\wd" + _INTL("Fin du combat.")) rescue nil
      end
    rescue; end
  end
